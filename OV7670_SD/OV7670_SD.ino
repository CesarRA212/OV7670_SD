...
void QVGA_Image(String title) {
  ...
  while (!(PIND & 8)); // Esperar a que sea alto
  while ((PIND & 8)); // Esperar a que sea bajo
  ...
  while (h--) {
    ...
    byte dataBuffer[320];
    while (w--) {
      while ((PIND & 4)); // Esperar a que sea bajo
      dataBuffer[319 - w] = (PINC & 15) | (PIND & 240);
      while (!(PIND & 4)); // Esperar a que sea alto
      while ((PIND & 4)); // Esperar a que sea bajo
      while (!(PIND & 4)); // Esperar a que sea alto
    }
...
void loop() {
  if(photoCount <= cantidadTomas) {
    String fileName = String(photoCount) + ".BMP";
    QVGA_Image(fileName);
    photoCount++;
  }
}
